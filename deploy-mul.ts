...
async function main() {
    try {
        ...
        await signer.login('sales@timechainlabs.io', 'string') // Securely handle credentials
        ...
        console.log(
            'Smart contract deployed: https://whatsonchain.com/tx/' +
                deployTx.id
        )

        const root = 2
        await new Promise((f) => setTimeout(f, 5000)) // Adjust timing as needed
